...
export default function BillPreview({ billData, onBack }) {
  ...
  return (
    <div style={{ padding: 20 }}>
      <button onClick={onBack} style={{ marginBottom: 10 }}  className="text-red-600 hover:text-red-800 font-semibold px-4 py-2 border border-red-600 rounded-xl transition">Back to Form</button>
      <button onClick={handlePrint} style={{ marginLeft: 10, marginBottom: 10 }}  className="text-red-600 hover:text-red-800 font-semibold px-4 py-2 border border-red-600 rounded-xl transition">Print Bill</button>
      ...
      >
        <div style={{ display: 'flex', justifyContent: 'space-between', borderBottom: '2px solid #1976d2', paddingBottom: 18, marginBottom: 24 }}>
          <div>
            <h1 style={{ color: '#1976d2', fontSize: '2.5rem', margin: 0, letterSpacing: 2 }}>
              Mobile Shop Mirzewala
            </h1> Main Market, Mirzewala, Sri Ganganagar
          </div>
        </div>

        <div style={{ display: 'flex', justifyContent: 'space-between', gap: 32, marginBottom: 12, fontSize: '1rem' }}>
          <div>
            <strong>Bill To:</strong> {billData.companyName}<br />
        ...
        <table style={{ width: '100%', borderCollapse: 'collapse', marginTop: 18, marginBottom: 18, background: '#fafbfc' }}>
          <thead>
            <tr style={{ background: '#e3eafc', color: '#1976d2', fontWeight: 600, fontSize: '1rem' }}>
        ...
        <div style={{ marginTop: 60, display: 'flex', justifyContent: 'space-between', alignItems: 'flex-end' }}>
          <div style={{ fontSize: '1.1rem', color: '#1976d2', fontStyle: 'italic' }}>
