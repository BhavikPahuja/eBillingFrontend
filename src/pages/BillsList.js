...
export default function BillsList() {
  ...
  const handleShowBill = async (billId) => {
    ...
    const res = await fetch(
      `https://ebillingbackend.onrender.com/api/bills/${billId}`
    );
    ...
    setSelectedBill({
      shopName: bill.shopName || "Rajasthan Mobile Shop",
      ...
      amountInWords:
        total > 0 ? `${toWords(total)} Rupees only` : "Zero Rupees only",
    });
  };

  const handleDownloadExcel = () => {
    window.open(
      "https://ebillingbackend.onrender.com/api/bills/excel",
      "_blank"
    );
  };
                  ...
                  {loading ? (
                    ...
                    bills.map((bill) => (
                      ...
                      >
                        ...
                        <td className="p-3">{bill.shopName || "N/A"}</td>
